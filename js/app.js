/* ==========================================================================
   Interactive Engine for LexPulse
   ========================================================================== */

// Base Simulated Article Database
const localArticles = [
    {
        "title": "Landmark Ruling Redefines Corporate Data Storage Liability",
        "excerpt": "A federal appellate court has established strict new standards for data breach negligence. Legal teams review immediate compliance shifts.",
        "category": "Corporate Law",
        "author": "Arthur Vance, Esq.",
        "date": "1 hour ago",
        "readTime": "5 min read",
        "body": "<p>A historic decision from the Second Circuit Court of Appeals has fundamentally shifted the burden of proof in data privacy disputes. The court ruled that companies failing to deploy multi-factor authentication and standard encryption protocols are presumptively negligent in the event of an external data breach.</p><p>Previously, plaintiffs had to prove that a specific security lapse directly enabled the intrusion. Now, general non-compliance with industry standards is sufficient to establish liability, making it significantly easier to certify data privacy class actions.</p><p>Corporate compliance officers are advising boards to immediately audit cybersecurity systems to ensure they align with the newly established judicial standards.</p>",
        "premium": true
    },
    {
        "title": "Patent Wars in Artificial Intelligence Hardware: Who Controls the Compute?",
        "excerpt": "Silicon giants clash in Delaware courtrooms over tensor processor architectures. We trace the patents that could define the industry's future.",
        "category": "Intellectual Property",
        "author": "Meera Joshi",
        "date": "3 hours ago",
        "readTime": "7 min read",
        "body": "<p>Litigation over proprietary semiconductor designs has escalated as computing power becomes the core commodity of the technology sector. Several startups and global chip designers have filed claims in Delaware federal courts, alleging patent infringements on advanced tensor and neural processing units.</p><p>At stake is the architecture that enables the rapid matrix multiplication necessary to run massive language and reasoning models. Leading hardware firms argue that competitors are duplicating energy-management and interconnect pipelines covered by patents from 2018.</p><p>A preliminary injunction could disrupt global supply chains for cloud computing providers. Legal experts anticipate that a cross-licensing settlement is the most likely outcome, though a prolonged court battle could establish historic IP precedents.</p>",
        "premium": false
    },
    {
        "title": "Personal Injury Law: How Autonomous Vehicle Claims Are Handled",
        "excerpt": "When self-driving vehicles crash, who pays? We explore the legal transitions from driver negligence to manufacturer product liability.",
        "category": "Litigation",
        "author": "Jonathan Sterling, Trial Attorney",
        "date": "5 hours ago",
        "readTime": "6 min read",
        "body": "<p>The integration of autonomous driving systems onto public roads is disrupting personal injury law, transforming typical driver negligence cases into complex product liability actions.</p><p>In standard collisions, liability centers on human driver error. However, when an autonomous vehicle (AV) operating under software control crashes, the focus shifts to hardware defects, sensor calibration issues, and software failure modes.</p><p>Plaintiffs' attorneys face high barriers to entry, requiring specialized engineers to inspect system logs and machine learning code. Manufacturers frequently argue that human occupants failed to intervene in accordance with the user agreement, establishing a complex framework of shared comparative fault.</p>",
        "premium": true
    },
    {
        "title": "FTC Unveils Stricter Anti-Trust Guidelines targeting Tech Mergers",
        "excerpt": "The Federal Trade Commission has announced comprehensive revisions to merger evaluations, expanding definitions of market dominance.",
        "category": "Regulatory",
        "author": "Helena Rostova",
        "date": "1 day ago",
        "readTime": "4 min read",
        "body": "<p>The Federal Trade Commission (FTC), alongside the Department of Justice, has released updated guidelines for evaluating corporate mergers and acquisitions. The new rules focus heavily on digital platform models and B2B software suites.</p><p>The updated guidelines expand the definition of anti-competitive behavior, analyzing whether an acquisition will serve to entrench a dominant platform's ecosystem or lock up key proprietary training data. The FTC emphasizes that even small transactions that buy out potential future rivals will face intense investigation.</p><p>M&A attorneys caution that regulatory reviews will extend significantly, advising tech firms to prepare comprehensive market analyses before initiating corporate buyouts.</p>",
        "premium": false
    },
    {
        "title": "The Rise of Legal Tech: Will AI Contracts Replace Junior Associates?",
        "excerpt": "Generative legal intelligence models can draft and review complex commercial leases in seconds. We explore the evolving role of corporate attorneys.",
        "category": "Legal Tech",
        "author": "Arthur Vance, Esq.",
        "date": "2 days ago",
        "readTime": "5 min read",
        "body": "<p>Law firms are adopting AI-enabled document analysis systems to streamline transaction workflows. These specialized models can cross-reference hundreds of agreements to flag compliance anomalies and negotiate standard terms instantly.</p><p>For clients, this technology represents dramatic cost reductions. Work that previously required dozens of billable hours from junior associates can now be processed in minutes. However, senior partners emphasize that human supervision remains essential to catch subtle contextual errors.</p><p>The legal industry is adapting by shifts in billing structures\u2014moving away from hourly models toward value-based billing\u2014and redesigning training programs for new attorneys.</p>",
        "premium": true
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isArticlePage) {
        if (artId !== null) {
            ...
            const savedRssArt = localStorage.getItem('2_lex_pulse_active_rss_article');
            const savedRssList = localStorage.getItem('2_lex_pulse_rss_articles');
            if (savedRssList) {
...
function loadStateFromStorage() {
    const savedTheme = localStorage.getItem('2_lex_pulse_dark_mode');
    state.isDarkMode = savedTheme === 'true';

    const savedBookmarks = localStorage.getItem('2_lex_pulse_bookmarks');
    if (savedBookmarks) {
        state.bookmarks = JSON.parse(savedBookmarks);
    }

    const savedUpvotes = localStorage.getItem('2_lex_pulse_upvotes');
    if (savedUpvotes) {
        state.upvoted = JSON.parse(savedUpvotes);
    }

    const savedComments = localStorage.getItem('2_lex_pulse_comments');
    if (savedComments) {
        state.comments = JSON.parse(savedComments);
    }

    const savedSub = localStorage.getItem('2_lex_pulse_subscribed');
    state.isSubscribed = savedSub === 'true';

    const savedViews = localStorage.getItem('2_lex_pulse_pageviews');
    if (savedViews) state.pageViews = parseInt(savedViews);

    const savedClicks = localStorage.getItem('2_lex_pulse_adclicks');
    if (savedClicks) state.adClicks = parseInt(savedClicks);

    const savedEarnings = localStorage.getItem('2_lex_pulse_earnings');
...
function saveStateToStorage() {
    localStorage.setItem('2_lex_pulse_dark_mode', state.isDarkMode);
    localStorage.setItem('2_lex_pulse_bookmarks', JSON.stringify(state.bookmarks));
    localStorage.setItem('2_lex_pulse_upvotes', JSON.stringify(state.upvoted));
    localStorage.setItem('2_lex_pulse_comments', JSON.stringify(state.comments));
    localStorage.setItem('2_lex_pulse_subscribed', state.isSubscribed);
    localStorage.setItem('2_lex_pulse_pageviews', state.pageViews);
    localStorage.setItem('2_lex_pulse_adclicks', state.adClicks);
    localStorage.setItem('2_lex_pulse_earnings', state.earnings.toFixed(2));
}
...
function renderTicker() {
    ...
    const items = ["Supreme Court schedules landmark arguments on digital end-user privacy", "Federal trade regulators block major tech buyout citing structural market concerns", "Patent litigation surges in high-performance neural computing design sectors", "Corporate compliance costs jump as new overseas privacy directives take effect", "Class-action filings hit record levels over employee data tracking grievances"];
...
function handleArticleClick(article) {
    ...
    if (localIdx > -1) {
        ...
        localStorage.setItem('2_lex_pulse_active_rss_article', JSON.stringify(article));
...
function runWorkspaceCalculator() {
    const resultEl = document.getElementById('calc-results');
            ...
            const area = document.getElementById('legal-area').value;
            const complexity = document.getElementById('case-complexity').value;
            
            let rate = 350;
            let baseHours = 20;
            let probability = 'Standard';
            ...
            if (complexity === 'medium') {
                multiplier = 2.5;
                probability = 'Moderate Complexity';
            } else if (complexity === 'high') {
                multiplier = 6.0;
                probability = 'High Litigation Risk';
            }
            ...
            resultEl.innerHTML = `
                <div class="calc-result-box" style="border-left: 4px solid var(--secondary); padding-left: 10px; margin-top: 15px;">
                    <h5 style="margin: 0 0 5px 0;">Case Evaluation:</h5>
                    <div style="font-size: 14px; margin-bottom: 5px;">Estimated Work: <strong>${hours} Billable Hours</strong></div>
                    <div style="font-size: 20px; font-weight: bold; color: var(--secondary);">$${fee.toLocaleString()} <span style="font-size: 12px; font-weight: normal; color: var(--text-color);">Est. Retainer Fee</span></div>
                    <div style="font-size: 11px; margin-top: 5px; opacity: 0.8;">Case Classification: <strong>${probability}</strong></div>
                    <p style="font-size: 10px; margin: 8px 0 0 0; opacity: 0.7; font-style: italic;">Disclaimer: Simulated estimate. Does not constitute legal advice. Actual attorney fees vary by firm and jurisdiction.</p>
                </div>
            `;
...
<a href="\${link}" target="_blank" class="hero-btn" style="display:inline-block; margin-top:10px;">Read Original Source <i class="fa-solid fa-external-link"></i></a>`
            ...
            localStorage.setItem('2_lex_pulse_rss_articles', JSON.stringify(parsedArticles));
